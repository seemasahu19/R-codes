...
cons_arima1 = arima(consumption, c(1,0,3), seasonal = c(1,0,1))

con_forecast = forecast(cons_arima1, h = 4 )

autoplot(con_forecast)
### values of p,d q

## auto.arima 
 
sov_arima = auto.arima(sovtrain, seasonal = T)

sov_ar = arima(sovtrain, c(0,1,2), seasonal =c(0,1,1))
...
rainfalltrain[ rainfalltrain > 500] = 500

rainfallmodel = HoltWinters(rainfalltrain)

rainfor = forecast(rainfallmodel, h = 12)

err = rainfalltest - rainfor$mean
...
rainar = arima(rainfalltrain, c(2,0,1), seasonal = c(1,0,1))

### forecasts for next 12 months 

rain_forec = forecast( rainar, h = 12)

rainforec = forecast(rain_forec, h=12)

## Model performance 

err = rainfalltest - rain_forec$mean
...
IOD_QBO = as.matrix(rainfall2[, 6:7])
...
IOD_QBO_fut = as.matrix(rainfall[132:137, 6:7])
rain.forecats2 = forecast(rainfall.arimax2, h = 6,xreg = IOD_QBO_fut )

rain.forecats2


err = rain2test - rain.forecats2$mean

mape = mean((abs(err)/rain2test)*100)


acf(rainfall.arimax2$residuals)
Box.test(rainfall.arimax2$residuals, lag=20, type="Ljung-Box")
